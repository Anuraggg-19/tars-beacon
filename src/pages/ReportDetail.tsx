...
import { updateReportStatus } from "@/lib/contract";

interface DecryptedReportDetail {
  category: string;
  location?: string;
  dateTime?: string;
  urgency: "low" | "medium" | "high" | "critical" | "critical";
  status: "pending" | "verified" | "flagged";
  description: string;
  media: { name: string; cid: string; type: string; size: number }[];
  cid: string;
  txHash?: string;
  timestamp?: Date;
  aiAnalysis: {
...
const ReportDetail = () => {
  ...
  const [report, setReport] = useState<DecryptedReportDetail | null>(null);
  ...
  useEffect(() => {
    const loadReport = async () => {
      ...
      try {
        ...
        const envelope = await fetchJsonFromIpfs<EncryptedReportEnvelope>(cid);
        const decrypted = await decryptReportEnvelope(envelope);

        // Basic AI / reputation placeholders
        const aiAnalysis = {
          credibilityScore: 80,
          keyEntities: [],
          recommendation:
            "Report successfully decrypted from IPFS. Integrate AI analysis here.",
        };

        setReport({
          category: decrypted.category || "Unknown",
          location: decrypted.location,
          dateTime: decrypted.dateTime,
          urgency: decrypted.urgency || "medium",
          status: "pending",
        ...
        console.error("Failed to load/decrypt report:", err);
        setError(
          err?.message ||
            "Unable to decrypt this report from IPFS. Please try again."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleMarkGenuine = async () => {
    ...
    await updateReportStatus(report.cid, "verified");
  ...
  const handleFlagSpam = async () => {
    ...
    await updateReportStatus(report.cid, "flagged");
        ...
        <main className="pt-20 pb-12">
          <div className="container mx-auto px-4">
            <p className="text-sm text-muted-foreground">
              Loading and decrypting report from IPFS...
            </p>
          </div>
        </main>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 animate-fade-in">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h1 className="text-3xl font-bold">{report.cid}</h1>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm">{report.location || "Unknown"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
